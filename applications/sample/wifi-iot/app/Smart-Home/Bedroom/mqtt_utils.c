...
int backup_wifi_connect(void) {
    ...
    strcpy(assoc_req.ssid, "qpt");        // WiFi名称
    strcpy(assoc_req.key, "qwertyuiop");     // WiFi密码
...
int mqtt_connect(void){
    ...
    // 配置MQTT连接参数
    data.clientID.cstring = "kitchen_device_01";  // 厨房设备唯一ID
    data.keepAliveInterval = 20;
    data.cleansession = 1;
    data.username.cstring = "kitchen_user";       // 可选：用户名
    data.password.cstring = "kitchen_pass";       // 可选：密码
